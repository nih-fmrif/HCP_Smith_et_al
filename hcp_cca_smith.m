...
EB=hcp2blocks('restricted_500_modified.csv', [ ], false, vars(:,1)); % change the filename to your version of the restricted file
...
%%% prepare main netmat matrix - we have a range of normalisation
%%% possibilities
...
for m = 0:19
    ...
    varNET = sum(var(NET));
    varYhat = sum(var(Yhat));
    
    modeVar(m+1) = 100*varYhat/varNET;
end

figure;
scatter(1:20,modeVar)
title('var plot for connectome edges')
...
for m = 0:19
    ...
    Yhat_noNaN = Yhat(:,all(~isnan(Yhat)));
    vars_noNaN = vars(:,all(~isnan(vars)));

    varVARS = sum(var(vars_noNaN));
    varYhat = sum(var(Yhat_noNaN));
    
%     
%     varVars = sum(var(vars));
%     varYhat = sum(var(Yhat));
    
    
    modeVar(m+1) = 100*varYhat/varVARS;
end

figure;
scatter(0:19,modeVar)
title('var plot for SMs')